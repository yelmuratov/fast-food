        ...
        @if ($createForm || $editForm)
            ...
            <div class="mb-3">
                ...
                <input type="number" wire:model.defer="monthly_salary_amount"
                    class="form-control @error('monthly_salary_amount') is-invalid @enderror"
            ...
            <div class="mb-3">
                ...
                <input type="number" wire:model.defer="bonus" class="form-control @error('bonus') is-invalid @enderror"
                    id="bonus">
                @error('bonus')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
                    ...
                    <tbody>
                        @foreach ($workers as $worker)
                            <tr>
                                ...
                                <td>
                                    <button type="button" wire:click="edit({{ $worker->id }})"
                                        class="btn btn-sm btn-warning">
                                        Edit
                                    </button>
                                    <button class="btn btn-sm btn-danger"
                                        wire:click="delete({{ $worker->id }})">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
